@extends('layouts.app')
@section('title', 'Sorties temporaires')
@section('header', 'Situation des sorties temporaires')

@section('content')
@include('mouvements._tabs')

<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
    <p class="text-sm text-gray-500">{{ $agents->total() }} agent(s) en sortie temporaire</p>
...
<form method="GET" class="card mb-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
    ...
    <div class="sm:col-span-2 flex gap-2">
        ...
        <a href="{{ route('mouvements.sorties-temporaires') }}" class="btn btn-secondary">Réinitialiser</a>
    </div>
</form>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        <thead>
            <tr class="text-left text-xs uppercase tracking-wide text-gray-500">
                ...
                <th class="table-head">Durée</th>
                <th class="table-head">Réf. de l'acte</th>
                <th class="table-head">Alerte</th>
                <th class="table-head">Date de reprise</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
            @forelse ($agents as $agent)
                <tr class="hover:bg-gray-50 {{ $agent->en_alerte ? 'bg-amber-50' : '' }}">
                    ...
                    <td class="px-4 py-3">
                        <span class="badge bg-amber-100 text-amber-800">{{ $agent->nature_sortie ?? '—' }}</span>
                    </td>
                    <td class="px-4 py-3 text-sm whitespace-nowrap">{{ $agent->date_sortie?->format('d/m/Y') ?? '—' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{{ $agent->duree_libelle }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500 font-mono">{{ $agent->reference_acte ?: '—' }}</td>
                    <td class="px-4 py-3 text-sm whitespace-nowrap">
                        @if ($agent->en_alerte)
                            <span class="badge bg-red-100 text-red-700" title="Fin prévue le {{ $agent->date_fin?->format('d/m/Y') }}">
                                ⚠ Fin {{ $agent->date_fin?->format('d/m/Y') }}
                            </span>
                        @elseif ($agent->date_fin)
                            <span class="text-gray-400">Fin {{ $agent->date_fin->format('d/m/Y') }}</span>
                        @else
                            <span class="text-gray-400">—</span>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-sm whitespace-nowrap">
                        @if ($agent->date_reprise)
                            <span class="text-green-700 font-medium">{{ $agent->date_reprise->format('d/m/Y') }}</span>
                        @else
            ...
            @empty
                <tr><td colspan="12" class="px-4 py-8 text-center text-gray-400">Aucun agent en sortie temporaire.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>

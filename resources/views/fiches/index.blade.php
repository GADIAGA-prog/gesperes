...
@section('header', 'Contrôle présence — Fiches de présence (A / B / C / D)')
@section('content')
...
@php
    $optAnnees = collect($annees)->mapWithKeys(fn($a) => [$a => $a]);
    $optTrim = [1 => '1er trimestre', 2 => '2e trimestre', 3 => '3e trimestre', 4 => '4e trimestre'];
@endphp
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
    ...
    <div class="card">
        ...
        <p class="text-xs text-gray-500 mb-3">Situation <strong>journalière</strong> d'une structure (cascade : services inclus).</p>
        <form method="GET" target="_blank" class="space-y-3" id="ficheA">
            <x-form.select name="structure_id" label="Structure" :options="$structures" required data-recherche />
    ...
    <div class="card">
        ...
        <p class="text-xs text-gray-500 mb-3">Situation <strong>mensuelle</strong> d'une structure — <strong>tout l'effectif</strong> (0 h / 0 j si aucune absence).</p>
        <form method="GET" target="_blank" class="space-y-3" id="ficheB">
            ...
            <x-form.select name="annee" label="Année" :options="$optAnnees" :selected="$annee" required />
            <div class="flex gap-2 pt-1">
    ...
    <div class="card">
        ...
        <p class="text-xs text-gray-500 mb-3">Situation <strong>mensuelle</strong> de <strong>tout le ministère</strong> (signée DRH). Effectif volumineux : préférez l'Excel.</p>
    ...
    {{-- FICHE D --}}
    <div class="card">
        <h3 class="font-semibold text-gray-800">Fiche D</h3>
        <p class="text-xs text-gray-500 mb-3">Situation <strong>trimestrielle</strong> — une structure, ou tout le ministère si non précisée.</p>
        <form method="GET" target="_blank" class="space-y-3" id="ficheD">
            <x-form.select name="structure_id" label="Structure" :options="$structures" placeholder="Tout le ministère" data-recherche />
            <x-form.select name="trimestre" label="Trimestre" :options="$optTrim" :selected="$trimestre" required />
            <x-form.select name="annee" label="Année" :options="$optAnnees" :selected="$annee" required />
            <div class="flex gap-2 pt-1">
                <button type="submit" formaction="{{ route('fiches.d') }}" name="format" value="pdf" class="btn btn-primary flex-1 justify-center">PDF</button>
                <button type="submit" formaction="{{ route('fiches.d') }}" name="format" value="xlsx" class="btn btn-secondary flex-1 justify-center">Excel</button>
            </div>
        </form>
    </div>
</div>
